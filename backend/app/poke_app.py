...
def list_pokemon(
    ...
    pokemons = repository.list_all(
        skip,
        limit,
        {"_id": False, "moveset": False}
    )
...
def list_moveset(
    ...
    pokemon = repository.list_one_pokemon(
        id=id,
        projection={"_id": False, "moveset": True}
    )
    ...
    raise HTTPException(
        status_code=404,
        detail=f"No moves found from pokemon {id}"
    )
...
def find_move(
    ...
    raise HTTPException(
        status_code=404,
        detail=f"Move {move_id} from pokemon {id} was not found"
    )
...
def create_pokemon(
    ...
    if isinstance(response, type(HTTPException(status_code=400))):
        raise response
